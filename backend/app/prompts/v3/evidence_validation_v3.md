# Evidence Validation / Critic Agent v3
...
2. **Advisor ownership**: the quoted turn is an advisor turn, OR an advisor clearly adopts the point in an adjacent turn (within at most 2 turns before or after). Do not infer endorsement from distant context. A statement about a *third party's* stance is not advisor ownership.
...
## Be especially careful to KEEP these strong-signal types

When all five gates above are met, the following types should rarely be rejected — only reject if the evidence concretely contradicts them:

- **Concrete consequence statements** (contractual penalty, comp impact, ownership constraint, compliance bind).
- **Contrast statements** showing specific differentiation between Optimize and the advisor's current situation.
- **Stated evaluation commitments** ("we're moving forward", "I'll take the weekend and decide").
- **Misfit declarations** ("we are a misfit for the models you have").
- **Stakeholder dependencies** (named third party must approve or weigh in).

If you are about to reject one of these, double-check that the rejection reason is grounded in the evidence — not just stylistic doubt.
...
- `third_party_stance_not_advisor_owned` — quote describes someone else's stance.
...
- `clarification_only` — question with no advisor motivation, constraint, or commitment revealed.
- `price_discovery_without_stated_reason` — clarification about cost or terms without a stated reason the economics matter.
...
When the evidence is borderline, reject — UNLESS the signal is one of the strong-signal types listed above (concrete consequence, contrast, commitment, misfit, stakeholder dependency). For those, when the evidence is borderline, keep and note the uncertainty in `validation_notes`. False negatives on strong signals are the costlier failure mode.
